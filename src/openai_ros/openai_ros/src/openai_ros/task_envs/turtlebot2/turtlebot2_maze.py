...
class TurtleBot2MazeEnv(turtlebot2_env.TurtleBot2Env):
    ...
    def discretize_observation(self,data,new_ranges):
        ...
        # selected_values = slice_list_symmetric(data.ranges, self.n_observations)

        for i, item in enumerate(data.ranges):
            # if (i%mod==0):
            if (i in {0, (len(data.ranges)/2)+1, len(data.ranges)-1}):
                ...
                # print 'self.min_range', self.min_range, 'item', item
                # print 'self._episode_done', self._episode_done    
